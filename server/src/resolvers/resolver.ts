...
export const Resolvers = {
    ...
    Mutation: {
        addNewUser: (parent: User, args: { newFirstName: string, newLastName: string, email: string, password: string }) => {
            return User.query().insert(
                {firstName: args.newFirstName, lastName: args.newLastName, email: args.email, password: args.password})
        },
        addNewCompany: (parent: Company, args: { userId: number, CompanyName: string }) => {
        ...
        addNewUserStory: async (parent: UserStory, args: { userId: number, projectId: number, userStory: string }) => {
            let newUserStory = await Project.relatedQuery('userStories').for(args.projectId)
